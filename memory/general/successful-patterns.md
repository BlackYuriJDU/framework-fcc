...
### [2026-08-08] Commit cirúrgico v7.1: add set explícito + secret-scan + .gitignore de runtime
- Contexto: Commit v7.1 num repo com 246 mudanças mistas (framework + lixo). HEAD era v5.0.0 (Fases 1-4); o working tree era o v7.1 completo. Precisava commitar só o escopo v7.1 sem vazar lixo nem credenciais.
- O que funcionou: (1) NUNCA `git add -A` — usar `git add` por caminhos explícitos (`.gitignore CLAUDE.md agents/ rules/ skills/ docs/ vertexion-agent-system/`) e `git reset -q <lixos>` para remover do stage o que caiu dentro (nul, __pycache__, token-metrics.md). (2) Antes do commit, rodar a ferramenta própria do framework `secret-scan.mjs` e confirmar que todos os achados eram untracked (file-history/, history.jsonl, plugins/) — 0 segredos commitados; o único "hit" tracked era placeholder de doc (`TUNNEL_TOKEN=PASTE_TUNNEL_TOKEN_HERE`). (3) Verificar a existência dos arquivos-chave do escopo no stage via `git diff --cached --name-only | grep <padrão>` (leads, control agents, skills). (4) `git commit` + `git tag -a v7.1.0`. (5) Descobriu que `git add <dir>` pega backups temporários gerados por scripts (orq.sh grava `registry.json.bak.*` em orchestrators/) — exigiu `git rm` + amend; prevenção = padrões de backup no `.gitignore` ANTES do commit. (6) O `.gitignore` foi expandido para cobrir artefatos de runtime com conteúdo sensível (history.jsonl, file-history/, session-env/, sessions/, shell-snapshots/, paste-cache/, cache/, telemetry/, jobs/, logs/, daemon/, tasks/, stats-cache.json) — mesmo racional da aprovação de `.credentials.json`/`framework-creds`.
- Padrão reaproveitável: para commit cirúrgico em repo com lixo, (a) add set por caminho explícito + reset dos intrusos; (b) validar segurança com secret-scan e separar achados untracked vs tracked; (c) antes de `git add`, checar se scripts do próprio repo geram `.bak` no mesmo dir e ignorá-los no `.gitignore`; (d) confirmar arquivos-chave staged por grep do `--cached --name-only`; (e) `git status` final deve ter só untracked fora de escopo.
