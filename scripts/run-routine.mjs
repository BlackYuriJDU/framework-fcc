...
const child = spawn(detected, ['-p', defaults[routineId] + '\nCreate/update the Task Contract and persist STATE.yaml before substantial work.', '--agent', 'control-tesla', '--model', 'opus', '--effort', 'high', '--permission-mode', 'default', '--output-format', 'stream-json', '--verbose', '--max-turns', '64'], { cwd: systemHome, stdio: 'inherit', env: { ...process.env, TZ: '(seu fuso)' } });
